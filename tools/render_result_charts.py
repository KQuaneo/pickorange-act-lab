...
def full_task(data: dict) -> str:
    ...
    ymax = 0.40
    out = [text(left, 42, "Final full-task success (20 episodes per checkpoint)", size=22, anchor="start", weight=500)]
    out.append(text(left, 68, "Bars: observed rate · whiskers: Wilson 95% interval · native horizon", size=13, anchor="start", fill="#9ca3af"))
...
def isolated(data: dict) -> str:
    ...
    legacy = {row["stage"]: row for row in rows if row["checkpoint_family"] == "legacy 7k"}
    final = {row["stage"]: row for row in rows if row["checkpoint_family"] == "14k"}
    width, height = 960, 500
    left, top, plot_w, plot_h = 92, 94, 808, 300
    ymax = 0.60
    out = [text(left, 42, "Isolated primitive success", size=22, anchor="start", weight=500)]
    out.append(text(left, 68, "20 episodes per cell · B2/B3 use oracle initialization", size=13, anchor="start", fill="#9ca3af"))
    ...
    for idx, stage in enumerate(["B1", "B2", "B3"]):
        center = left + group_w * (idx + 0.5)
        for row, x, color in [(legacy[stage], center - 34, "#a78bfa"), (final[stage], center + 34, "#34d399")]:
            rate = row["rate"]
            y = top + plot_h * (1 - rate / ymax)
            base = top + plot_h
            out.append(f'<rect x="{x-25:.1f}" y="{y:.1f}" width="50" height="{base-y:.1f}" rx="5" fill="{color}"/>')
            out.append(text(x, y - 10, f'{row["successes"]}/20', size=14, weight=500))
        label = stage if stage == "B1" else f"{stage} (oracle init)"
        out.append(text(center, top + plot_h + 32, label, size=14))
    out.append(f'<rect x="{left + 570}" y="45" width="14" height="14" rx="3" fill="#a78bfa"/>')
    out.append(text(left + 592, 57, "legacy 7k", size=13, anchor="start"))
    out.append(f'<rect x="{left + 685}" y="45" width="14" height="14" rx="3" fill="#34d399"/>')
    out.append(text(left + 707, 57, "14k", size=13, anchor="start"))
    out.append(text(left + plot_w / 2, height - 42, "Primitive policy", size=13, fill="#9ca3af"))
    return document(
        "Isolated primitive success rates",
        "Legacy 7k versus 14k primitive checkpoints. B2 and B3 use oracle initialization and are capability upper bounds.",
        out,
        width,
        height,
    )
